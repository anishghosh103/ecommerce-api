...
let customerAuthenticated = (req, res, next) => {
  ...
  if (!token) {
    res.json(response.error('You are not authorized to access this page. Login first.', 403));
  } else {
...
let adminAuthenticated = (req, res, next) => {
  const token = req.cookies.token;
  if (!token) {
    res.json(response.error('Admin Permission Required. You are not authorized to access this page.', 403));
  } else {
    jwt.verify(token, appConfig.secret, (err, decoded) => {
